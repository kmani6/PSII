...
%     fprintf('train %i \n', train)
    for flash = 1:n_flashes
        ...
        jd40_index = find(strcmp(knames, 'jd40') == 1);
        ...
        oqr = (k(oqr_index)*PQ);
        %rqd = (k(rqd_index)*Hs);
        jd40 = (k(jd40_index)*H20_Substrate);
...
%         legend(sorted_species(S3_sorted))
        ...
        n_timepoints = flash_interval*10000;

%         Soly1 = ode1(@(t,y) PS2ODES(t,y,k(kconst),k,rate_inds,S,Rknames,species),linspace(0,flash_interval, n_timepoints),yinitial);
%         Soly2 = ode2(@(t,y) PS2ODES(t,y,k(kconst),k,rate_inds,S,Rknames,species),linspace(0,flash_interval, n_timepoints),yinitial);
%         Soly3 = ode3(@(t,y) PS2ODES(t,y,k(kconst),k,rate_inds,S,Rknames,species),linspace(0,flash_interval, n_timepoints),yinitial);
%         Soly4 = ode4(@(t,y) PS2ODES(t,y,k(kconst),k,rate_inds,S,Rknames,species),linspace(0,flash_interval, n_timepoints),yinitial);
%         Soly5 = ode5(@(t,y) PS2ODES(t,y,k(kconst),k,rate_inds,S,Rknames,species),linspace(0,flash_interval, n_timepoints),yinitial);
        Sol = ode15s(@(t,y) PS2ODES(t,y,k(kconst),k,rate_inds,S,Rknames,species),t_lims,yinitial);
%         save(['tmp_files\species_concs_vars'], 'k','kconst','rate_inds','S','Rknames','species','t_lims','yinitial')
        ...
        k2 = k;
        for ii = 2:n_timepoints+1
            k2(rqr_index) = rqr(ii-1);
            k2(oqr_index) = oqr(ii-1);
            %k2(rqd_index) = rqd(ii);
            k2(jd40_index) = jd40(ii-1);
            k2(delta32_index) = delta32(ii-1);
            for i = 1:length(adj)
                Xinds = find(X == i);
                kconstind = kconst(Xinds);
                kconstval = k2(kconstind);
